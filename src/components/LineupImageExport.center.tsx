...
// Centered 9:16 variant — kept for later; swap the import in MatchLineupBuilder to use it
export default function LineupImageExportCenter({ slots, format, match, venue, ourTeamId, cutouts = {} }: Props) {
  ...
  if (story) {
    ...
    const section = (label: string, rows: LineupSlot[][], rowWidth = '100%') => (
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 26, minWidth: 0, width: '92%', margin: '0 auto' }}>
          {rows.map((row, ri) => (
